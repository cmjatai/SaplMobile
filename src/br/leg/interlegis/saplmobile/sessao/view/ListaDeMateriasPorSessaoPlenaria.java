...
public class ListaDeMateriasPorSessaoPlenaria extends LinearLayout implements View.OnTouchListener{
		...
		public void onReceive(Context context, Intent intent) {
			...
			try {
				...
				if (intent.getAction().equals(PROCESS_UPDATE_MATERIAS_SESSAO)) { 
					...
					TextView textInformaDownload = (TextView) fragment.getActivity().findViewById(R.id.textInformaDownload); 

					if (arquivosParaBaixar != 0)
						textInformaDownload.setText(arquivosParaBaixar+" Arquivos a serem baixados...");
					else 
						textInformaDownload.setText("");
					...
					TextView textInformaAtualizacao = (TextView) fragment.getActivity().findViewById(R.id.textInformaAtualizacao); 

					textInformaAtualizacao.setText("Próxima Atualização em aproximadamente "+proximaAtualizacao+"s");
	...
	public final void requestMats(String xmlMats) throws Exception {
		...
		TextView textInforma = (TextView) fragment.getActivity().findViewById(R.id.textInformaAtualizacao); 

		textInforma.setText("Atualizando...");
		...
		if (nlMatAnexadas.getLength() == 1) {
			...
			if (nlMatAnexadas.getLength() == 0) {
				...
				for (int i = 0; i < nlMatAnexadas.getLength(); i++) {
					...
					nlAutores = elMatAnex.getElementsByTagName("autores-matanex");
